...
int main(int argc, char **argv)
{
    ...
    //ファイル読み込みここから
    FILE *fp;
    fp = fopen("/Users/ats/Desktop/数理工学実験/20211213/kadai13/mmse_kadai6.txt", "r");
    if (fp == NULL)
    {
    ...
    double V[2][2][2] = {{{100.0, 0}, {0, 1.0}}, {{2.0, 0}, {0, 1.0}}}; //誤差の共分散行列
    double Q[2][2][2] = {{{0.01, 0}, {0, 1.0}}, {{0.5, 0}, {0, 1.0}}};  //重み行列（Vの逆行列）を格納する配列
    int s;                                                              //配列Qに用いる
    ...
    for (e = 0; e < N; e++)
    {
        ...
        for (int k = 0; k < p; k++)
        {
            for (int l = 0; l < p; l++)
            {
                ...
                for (int m = 0; m < p; m++)
                {
                    phiQ1[k][l] = phiQ1[k][l] + phi[m][k] * Q[s][m][l];
                }
            }
        }
        for (int k = 0; k < p; k++)
        ...
        for (int k = 0; k < p; k++)
        {
            for (int l = 0; l < p; l++)
            {
                ...
                for (int m = 0; m < p; m++)
                {
                    phiQ2[k][l] = phiQ2[k][l] + phi[m][k] * Q[s][m][l];
                }
            }
        }
        for (int k = 0; k < p; k++)
